...
To regenerate the figures used in the submitted document and the demo video, run the evaluation notebook on the **pre-denoised** H5 stack(s):

- **Download denoised H5 stack(s)** (not stored in this repo due to size):
  - Cloud link: `https://nextcloud.mpie.de/s/iFxLxtWXn86Tsee`
  - Password: `Dxy5WSKbjc`
  - Files available:
    - `denoised_100epochs_babyunet.h5`
    - `denoised_100epochs_dncnn.h5`
    - `denoised_100epochs_unet.h5`
- **Place the file(s)** into `data/` (e.g. `data/denoised_100epochs_babyunet.h5`)
- **Run the notebook**: `evaluation_notebooks/SmallUNET_evaluation.ipynb`
  - This notebook loads the denoised stack, computes the strain ROI maps, and generates the CoM overlay plots.
  - By default it is configured for **BabyUNet** (fastest training during the hackathon); you can switch to DnCNN/UNet by changing the denoised filename in the notebook’s Paths cell.
